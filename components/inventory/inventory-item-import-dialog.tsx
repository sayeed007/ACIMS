...
import { useBulkImportInventoryItems } from '@/hooks/useInventoryItems';

interface InventoryItemImportDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function InventoryItemImportDialog({ open, onOpenChange }: InventoryItemImportDialogProps) {
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [importResult, setImportResult] = useState<any>(null);

  const bulkImportMutation = useBulkImportInventoryItems();
  ...
  const handleDownloadTemplate = async () => {
    try {
      const response = await fetch('/api/inventory/items/template', {
        method: 'GET',
        headers: {
      ...
      a.href = url;
      a.download = `inventory_items_import_template.xlsx`;
    ...
    try {
      ...
      if (error.response?.data?.error?.details) {
        setImportResult({
          total: 0,
          imported: 0,
          failed: error.response.data.error.details.length,
  ...
  const getStatusColor = () => {
    ...
    if (importResult.imported > 0) return 'default';
    return 'destructive';
  };
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload an Excel file to import multiple inventory items at once. Download the template to get started.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          {/* Success List */}
          {importResult && importResult.items && importResult.items.length > 0 && (
            <div className="space-y-2">
              <h3 className="font-semibold text-green-600">Successfully Imported Items</h3>
              <div className="border rounded-lg max-h-60 overflow-y-auto">
                <Table>
                  <TableHeader>
                    <TableRow>
                  ...
                  <TableBody>
                    {importResult.items.map((item: any) => (
          ...
          <Alert>
            ...
            <AlertDescription className="text-sm space-y-1">
              <ul className="list-disc list-inside space-y-1">
                ...
                <li>Item Code and Unit will be automatically converted to uppercase</li>
                <li>Status must be either ACTIVE or INACTIVE</li>
                <li>Total Value is calculated automatically (Current Stock × Avg Cost Per Unit)</li>
