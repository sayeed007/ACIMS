...
export function DemandFormDialog({
  ...
  const onSubmit = async (data: DemandFormData) => {
    ...
    try {
      const payload = {
        // Only include demandNumber when editing (for create, backend will auto-generate)
        ...(mode === 'edit' && demand?.demandNumber ? { demandNumber: demand.demandNumber } : {}),
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                disabled
                className="bg-muted"
                value={mode === 'edit' ? demand?.demandNumber : ''}
                readOnly
              />
              <p className="text-xs text-muted-foreground">
                ℹ️ Demand number will be automatically generated by the system
              </p>
            </div>
