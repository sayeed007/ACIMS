...
export function PurchaseOrderFormDialog({
  ...
  const onSubmit = async (data: POFormData) => {
    ...
    try {
      const payload = {
        // Only include poNumber when editing (for create, backend will auto-generate)
        ...(mode === 'edit' && order?.poNumber ? { poNumber: order.poNumber } : {}),
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                disabled
                className="bg-muted"
                value={mode === 'edit' ? order?.poNumber : ''}
                readOnly
              />
              <p className="text-xs text-muted-foreground">
                ℹ️ PO number will be automatically generated by the system
              </p>
            </div>
