...
export async function GET(request: NextRequest) {
  try {
    ...
    return errorResponse('INTERNAL_ERROR', error.message || 'Failed to fetch rules', null, 500)
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.ruleName || !body.mealSession) {
      return validationError('Rule name and meal session are required')
    }
    ...
    return errorResponse('INTERNAL_ERROR', error.message || 'Failed to create rule', null, 500)
  }
}
